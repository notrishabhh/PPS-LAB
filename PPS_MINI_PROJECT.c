...
int regist_status(long entered_adhar)
{
    //Checks for registration details of new patients
    patient_log[0].aadhar=entered_adhar;
    int i=1;
    while(i<=1000)
    {
        if(patient_log[i].aadhar==patient_log[0].aadhar)
        {
            ...
            return 0;  //this means registered but not logged in
            else 
            return 1;  //registered and logged in
        }
        i++;
    }
...
{
    if(patient_log[i].vaccine_doses<2){
         patient_log[i].vaccine_doses++;
        return patient_log[i].vaccine_doses;
        ...
        apply:
            ...
            while(i<=1000)
            {
                if(patient_log[i].aadhar==entred_adhar)
                   j=i;   //for storing the index of stored number
                   i++;
            }
            ...
            nxt2:
                ...
                if(vstat==1)
                   ...
                   //Count of total number of vaccines vials used
